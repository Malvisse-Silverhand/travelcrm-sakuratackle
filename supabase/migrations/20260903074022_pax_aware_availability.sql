-- The public calendar renders ONE cell per date (green/amber/red), but the
-- question it actually answers is "can this night take MY group of N?" — which
-- the per-boat rollup can't express. The design mock faked this with a hash
-- (`statusFor()` in Laman Tempahan.dc.html), so there was no spec to copy.
--
-- This replaces get_public_availability with a pax-aware, per-night version:
--   full         = no un-blocked boat has N seats left
--   selling_fast = only one boat can still take the group, or the fleet is
--                  >=70% held for that night (mirrors the 0.7 threshold the
--                  build prompt uses for trip_night_status)
--   open         = otherwise
--
-- It also returns suggested_boat_id — the tightest-fitting boat, so groups pack
-- efficiently instead of each taking a fresh boat — which the booking form needs
-- to pass to create_public_booking. Deliberately returns no raw pax counts.
...
language sql security definer stable set search_path = public as $$
  ...
  best as (
    ...
    from per_boat
    where not blocked and seats_left >= p_pax
    order by night_date, seats_left asc, boat_code asc
  ),
  agg as (
    select
      night_date,
      count(*) filter (where not blocked and seats_left >= p_pax) as fitting_count,
